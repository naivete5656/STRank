...
def train_model(
    ...
    data_dict = datasets.load_patch_data(data_dir)
    if args.use_gene:
        use_gene_list = np.loadtxt(args.use_gene, dtype=str).tolist()
    else:
        use_gene_list = None
    if no_sample_wise:
        test_dataset, val_dataset, train_dataset = datasets.generate_datasets_all_samples(
            data_dict, val_ratio=0.1, test_ratio=0.1, use_gene_list=use_gene_list
        )
    else:
        test_dataset, val_dataset, train_dataset = datasets.generate_datasets_slide_wise(
            data_dict,
    ...
    val_loader = torch.utils.data.DataLoader(val_dataset, collate_fn=collate_fn, batch_size=1, shuffle=False)
    # setup log setting
    loss_key = loss
    model_key = model

    _eps = 1e-6
    # mean_count_w = 1 / (torch.tensor([count["count"].mean() for count in train_dataset]).mean() + _eps)
    ...
    model = hist2gene.His2Gene(
        model_key,
        loss_key,
        model_params={
            # "in_features": train_dataset[0]["img_feat"].shape[0],
            "in_features": 10,
            "out_features": len(use_gene_list),
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=f"{log_dir}/ckpt", monitor=monitor_loss)
    # setup trainer
    ...
    trainer.fit(model, val_dataloaders=val_loader, train_dataloaders=train_loader)
    model = hist2gene.His2Gene.load_from_checkpoint(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log_dir",
        type=str,
        default="./dataset/hest1k/st_v2/opts/comp/logs",
        help="Directory to save logs and checkpoints",
    )
    parser.add_argument("--loss", type=str, help="Loss function to use", default="stranklist")
    parser.add_argument("--model", type=str, help="Model architecture to use", default="linear")
    parser.add_argument("--max_epochs", type=int, default=100, help="Maximum number of epochs to train")
    parser.add_argument("--ngpu", type=int, default=1, help="Number of GPUs to use for training")
    parser.add_argument(
        "--use_gene",
        type=str,
    ...
    args.val_sample_ids = ["SPA142", "SPA141", "SPA140", "SPA139", "SPA138", "SPA137"]
    train_model(
        args.data_dir,
        args.param_path,
        args.log_dir,
